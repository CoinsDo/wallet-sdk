...
// with the passed bsv cash network.
...
func ParsePkScript(pkScript []byte, net *chaincfg.Params) (string, error) {
	if len(pkScript) < 3 {
	...
	case len(pkScript) == 25 && pkScript[0] == 0x76 && pkScript[1] == 0xa9 && pkScript[2] == 0x14 && pkScript[23] == 0x88 && pkScript[24] == 0xac:
		publicKeyHash := pkScript[3:23]
		return EncodeHash(publicKeyHash, NetList[net.Name].PubHashPrefixes)
	case len(pkScript) == 23 && pkScript[0] == 0xa9 && pkScript[1] == 0x14 && pkScript[22] == 0x87:
		scriptHash := pkScript[2:22]
		return EncodeHash(scriptHash, NetList[net.Name].PubHashPrefixes)
	default:
		return "", fmt.Errorf("Unknown PkScript type")
	}
}
func P2PKHAddressFromPkScript(pkScript []byte, net *chaincfg.Params) string {
	if len(pkScript) != 25 || pkScript[0] != 0x76 || pkScript[1] != 0xa9 || pkScript[2] != 0x14 || pkScript[23] != 0x88 || pkScript[24] != 0xac {
		return "Invalid P2PKH PkScript"
	}

	publicKeyHash := pkScript[3:23]
	zecAddress, _ := EncodeHash(publicKeyHash, NetList[net.Name].PubHashPrefixes)
	return zecAddress
}
